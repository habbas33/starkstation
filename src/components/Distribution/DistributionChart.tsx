...
export default function DistributionChart(props: {
    ...
    const { data,isLoading, chain, variance } = props;
  ...
  return (
    <div className={`h-60 w-full drop-shadow-xl w-full ${chain==="ethereum" ? "bg-cyan-900":"bg-cyan-900"} rounded-3xl rounded-2xl p-5 self-end`}>
        <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
                <h1 className="text-gray-300 text-sm"> Variance (last 50 blocks):</h1>
                <h1 className="text-gray-200 font-semibold text-sm pl-1">{isNaN(variance)?"":toK(variance)}</h1>
            </div>
        </div>
        {!isLoading ? (
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip
                  ...
                  labelFormatter={label => toKe(label)}
                  ...
                  />
                <Line type="monotone" dataKey="normalDensityZx" isAnimationActive={false} stroke={chain==='ethereum'?'#0ea5e9':'#fb7185'} dot={false}  />
              </LineChart>
            </ResponsiveContainer>
        ) : (
...
export const toKe = (num:any) => {
  return 'BlockTime : ' + Numeral(num).format('0.[0000]a') + ' (sec)';
};
