...
export default function TpbChart(props: {
    data:any
    isLoading:boolean
    chain: string
    avgTpb: number
}) {
    const { data,isLoading, chain, avgTpb } = props;
  ...
  return (
    <div className={`h-60 w-full drop-shadow-xl w-full ${chain==="ethereum" ? "bg-cyan-900":"bg-cyan-900"} rounded-3xl rounded-2xl p-5 self-end`}>
        <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
                <h1 className="text-gray-300 text-sm"> Average TPB (last 50 blocks):</h1>
                <h1 className="text-gray-200 font-semibold text-sm pl-1">{isNaN(avgTpb)?"":avgTpb}</h1>
            </div>
        </div>
        {!isLoading ? (
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip
                  ...
                  />
                <Line type="monotone" dataKey="txnCount" isAnimationActive={false} stroke={chain==='ethereum'?'#0ea5e9':'#fb7185'} dot={false}  />
              </LineChart>
            </ResponsiveContainer>
        ) : (
